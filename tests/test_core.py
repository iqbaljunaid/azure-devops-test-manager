...
import json
import os
from azure_devops_test_manager.core import AzureTestPointManager, ConfigurationError, AzureAPIError
...
class TestAzureTestPointManager:
    ...
    def test_update_test_point_outcome_success(self, mock_patch):
        ...
        manager = AzureTestPointManager()
        result = manager.update_test_point_outcome(12345, 67890, 101, 'Passed', 'Test comment')
...
class TestFuzzyMatching:
    ...
    def test_fuzzy_match_test_names(self):
        ...
        matches = manager.fuzzy_match_test_names(test_results, azure_test_points, min_score=70)
        
        # Should find some matches
        assert len(matches['matches']) >= 0  # May vary based on fuzzy matching algorithm
