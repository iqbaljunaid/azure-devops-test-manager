...
class AzureTestPointManager:
    ...
    def get_test_points(self, plan_id: int, suite_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise AzureAPIError(f"HTTP Error fetching test points for suite {suite_id}: {e}")
        except Exception as e:
        ...
        try:
            ...
            if steps_field:
                ...
                for step in soup.find_all("step"):
                    ...
                    action = parameterized_strings[0].text if len(parameterized_strings) > 0 else ""
                    expected = parameterized_strings[1].text if len(parameterized_strings) > 1 else ""
            ...
            return {
                ...
                'automation_status': fields.get('Microsoft.VSTS.TCM.AutomationStatus', 'Not Automated'),
                'steps': steps,
    ...
    def process_test_point(self, point: Dict[str, Any], detailed: bool = False) -> Dict[str, Any]:
        """
        ...
        if detailed and processed_point['test_case_id']:
            test_case_details = self.get_test_case_details(processed_point['test_case_id'])
            processed_point.update({
                'test_case_details': test_case_details,
                'test_case_title': test_case_details.get('title', processed_point['test_case_name']),
                'test_case_state': test_case_details.get('state', 'Unknown'),
                'test_case_priority': test_case_details.get('priority', 'Unknown'),
                'automation_status': test_case_details.get('automation_status', 'Not Automated'),
                'steps_count': len(test_case_details.get('steps', []))
            })
        
        return processed_point
    
    def list_test_points_for_plan(self, 
                                  plan_id: int, 
        ...
        if suite_id:
            ...
            if test_points:
                processed_points = [self.process_test_point(point, detailed) for point in test_points]
    ...
    def update_test_point_outcome(self, 
                                  plan_id: int, 
                                  suite_id: int, 
                                  point_id: int, 
                                  outcome: str, 
        ...
        try:
            ...
            response = requests.patch(url, auth=self.auth, headers=self.headers, json=payload)
            response.raise_for_status()
    ...
    def parse_test_results_xml(self, xml_file_path: str) -> Dict[str, List[Dict[str, Any]]]:
        """
    ...
    def fuzzy_match_test_names(self, 
        ...
        for azure_point in all_azure_points:
            azure_name = azure_point.get('test_case_title', azure_point['test_case_name'])
            ...
            match_result = process.extractOne(azure_clean_name, xml_test_names, scorer=fuzz.ratio)
            ...
            match_result = process.extractOne(azure_clean_name, xml_full_names, scorer=fuzz.partial_ratio)
            if match_result and match_result[1] > best_score:
                best_match = match_result
                best_score = match_result[1]
            ...
            for xml_test in all_test_results:
                ...
                score = fuzz.token_sort_ratio(azure_clean_name.lower(), xml_name.lower())
                if score > best_score:
            ...
            if best_score >= min_score:
                ...
                if match_strategy == "clean_name":
                    matched_xml_test = next((t for t in all_test_results if t['clean_name'] == best_match[0]), None)
                elif match_strategy == "full_name":
                    matched_xml_test = next((t for t in all_test_results if t['full_name'] == best_match[0]), None)
                else:  # token_sort
                    matched_xml_test = next((t for t in all_test_results if t['clean_name'] == best_match[0]), None)
        ...
        unmatched_xml = [test for test in all_test_results if test['name'] not in matched_xml_names]
    ...
    def update_from_test_results(self, 
                                 plan_id: int, 
                                 xml_file_path: str, 
                                 suite_id: Optional[int] = None, 
                                 min_score: int = 80, 
        ...
        matching_results = self.fuzzy_match_test_names(test_results, azure_test_points, min_score)
